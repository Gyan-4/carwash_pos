...
export default function POSInterface() {
  ...
  useEffect(() => {
    ...
    const loadPromos = async () => {
      ...
      try {
        ...
        if (!cancelled) setPromos((data.promos || []).filter((promo: Promo) => promo.active));
      } catch {
  ...
  // Vehicle/size eligibility determines which promos are selectable.
  // Platform and verification are requirements after a promo is selected.
  ...
  }, [selectedPromo, riderPlatform, promoVerification]);

  useEffect(() => {
    if (selectedPromo && !eligiblePromos.some((promo) => promo._id === selectedPromo._id)) {
      setSelectedPromo(null);
      setRiderPlatform('');
      setPromoVerification(false);
    }
  }, [eligiblePromos, selectedPromo]);

  const discount = useMemo(() => {
    if (!selectedPromo || !promoRequirementsMet) return 0;
    if (selectedPromo.discountType === 'percentage') {
      return Math.min(subtotal, subtotal * (selectedPromo.discountValue / 100));
    }
    ...
    try {
      const response = await fetch('/api/transactions', {
        ...
        body: JSON.stringify({
          ...
          promoName: selectedPromo?.name || undefined,
          paymentMethod,
          amountPaid: paidNumber,
          riderPlatform: riderPlatform || undefined,
      ...
      setReceipt({
        ...
        vehicleType,
        vehicleSize: vehicleType === 'motorcycle' ? undefined : vehicleSize,
        services: selectedServices.map((service) => ({ name: service.name, price: getPrice(service, vehicleType, vehicleSize) })),
        subtotal,
        discount,
        total,
        paymentMethod,
    ...
    <div className="w-full h-full flex gap-4 p-4 overflow-hidden bg-slate-50/60 text-slate-900">
      ...
      <div className="w-80 shrink-0 h-full bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col">
        ...
        <div className="border-t border-slate-200 p-4 space-y-3">
          <div className="flex items-center justify-between"><h3 className="text-xs font-extrabold text-slate-800">Promo / Discount</h3>{loadingPromos && <span className="text-[9px] text-slate-400">Loading...</span>}</div>
          <select value={selectedPromo?._id || ''} onChange={(e) => { const promo = eligiblePromos.find((item) => item._id === e.target.value) || null; setSelectedPromo(promo); setRiderPlatform(''); setPromoVerification(false); }} disabled={eligiblePromos.length === 0} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-[11px] font-bold text-slate-800"><option value="">No promo</option>{eligiblePromos.map((promo) => <option key={promo._id} value={promo._id}>{promo.name} — {promo.discountType === 'percentage' ? `${promo.discountValue}% OFF` : `₱${promo.discountValue.toLocaleString()} OFF`}</option>)}</select>
          {eligiblePromos.length === 0 && promos.length > 0 && <p className="text-[10px] text-slate-500">No configured promos are currently eligible for this vehicle.</p>}
          {selectedPromo && <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 space-y-2"><div className="text-[11px] font-extrabold text-blue-900">{selectedPromo.name}</div>{selectedPromo.description && <div className="text-[10px] leading-4 text-blue-700">{selectedPromo.description}</div>}
            {selectedPromo.eligiblePlatforms && selectedPromo.eligiblePlatforms.length > 0 && <select value={riderPlatform} onChange={(e) => setRiderPlatform(e.target.value)} className="w-full rounded-lg border border-blue-200 bg-white px-2.5 py-2 text-[10px] font-bold text-slate-800"><option value="">Select rider platform</option>{RIDER_PLATFORMS.filter((platform) => selectedPromo.eligiblePlatforms?.includes(platform)).map((platform) => <option key={platform} value={platform}>{platform}</option>)}</select>}
