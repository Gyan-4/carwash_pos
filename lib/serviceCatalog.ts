...
export type ServiceComponent = 'wash' | 'vacuum' | 'tire-black' | 'interior-dressing' | 'engine-wash' | 'wax' | 'acidrain-removal';
export interface CatalogItem { id: string; name: string; category: string; prices?: Partial<Record<VehicleSize, number>>; motorcyclePrice?: number; description?: string; components?: ServiceComponent[]; }
const carWash = (id: string, name: string, prices: [number, number, number, number, number], components: ServiceComponent[]): CatalogItem => ({ id, name, category: 'Car Wash Packages', prices: { small: prices[0], medium: prices[1], large: prices[2], xl: prices[3], xxl: prices[4] }, components });
...
export const MOTORCYCLE_SERVICES: CatalogItem[] = [
  {id:'mc-wash',name:'Wash',category:'Motorcycle',motorcyclePrice:100,components:['wash']}, {id:'mc-dressing',name:'Dressing',category:'Motorcycle',motorcyclePrice:90,components:['interior-dressing']}, {id:'mc-wax',name:'Wax',category:'Motorcycle',motorcyclePrice:100,components:['wax']}, {id:'mc-tire-black',name:'Tire Black',category:'Motorcycle',motorcyclePrice:60,components:['tire-black']},
  {id:'mc-wash-dressing',name:'Wash + Dressing',category:'Motorcycle',motorcyclePrice:160,components:['wash','interior-dressing']}, {id:'mc-wash-dressing-tire',name:'Wash + Dressing + Tire Black',category:'Motorcycle',motorcyclePrice:200,components:['wash','interior-dressing','tire-black']}, {id:'mc-wash-dressing-wax',name:'Wash + Dressing + Wax',category:'Motorcycle',motorcyclePrice:220,components:['wash','interior-dressing','wax']}, {id:'mc-wash-dressing-tire-wax',name:'Wash + Dressing + Tire Black + Wax',category:'Motorcycle',motorcyclePrice:240,components:['wash','interior-dressing','tire-black','wax']},
];
...
export const VEHICLE_SIZES = [{id:'small',label:'Small'},{id:'medium',label:'Medium'},{id:'large',label:'Large'},{id:'xl',label:'XL'},{id:'xxl',label:'XXL'}] as {id:VehicleSize;label:string}[];
export function getPrice(item: CatalogItem, vehicleType: VehicleType, vehicleSize: VehicleSize): number { return vehicleType === 'motorcycle' ? item.motorcyclePrice ?? 0 : item.prices?.[vehicleSize] ?? item.prices?.small ?? 0; }
export function hasPrice(item: CatalogItem, vehicleType: VehicleType, vehicleSize: VehicleSize): boolean { return vehicleType === 'motorcycle' ? item.motorcyclePrice !== undefined : item.prices?.[vehicleSize] !== undefined || item.prices?.small !== undefined; }
function sameComponents(a: ServiceComponent[] = [], b: ServiceComponent[] = []): boolean { const left=[...new Set(a)].sort(), right=[...new Set(b)].sort(); return left.length===right.length && left.every((v,i)=>v===right[i]); }
export function findPackageUpgrade(selected: CatalogItem[], vehicleType: VehicleType, vehicleSize: VehicleSize): CatalogItem | undefined { const components=[...new Set(selected.flatMap(i=>i.components??[]))]; if(components.length<2)return undefined; const packages=vehicleType==='motorcycle'?MOTORCYCLE_SERVICES:[...CAR_WASH_PACKAGES,...PREMIUM_PACKAGES]; const matches=packages.filter(i=>hasPrice(i,vehicleType,vehicleSize)&&sameComponents(i.components,components)); return matches.length===1?matches[0]:undefined; }
export function packageContainsSelection(packageItem: CatalogItem, selected: CatalogItem[]): boolean { const selectedComponents=[...new Set(selected.flatMap(i=>i.components??[]))]; return selectedComponents.length>0&&selectedComponents.every(c=>packageItem.components?.includes(c)); }
export function conflictsWithSelection(_item: CatalogItem, _selected: CatalogItem[]): CatalogItem | undefined { return undefined; }
