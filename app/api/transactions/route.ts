...
export async function POST(req: Request) {
  try {
    ...
    if (!Number.isFinite(subtotal) || subtotal < 0) return NextResponse.json({ success: false, error: 'Invalid transaction subtotal.' }, { status: 400 });
    await connectToDatabase();
    ...
    if (body.promoId) {
      ...
      if (!promo || !promoMatches(promo, body, body.services)) return NextResponse.json({ success: false, error: 'Selected promotion is not eligible for this order.' }, { status: 400 });
      if (promo.requiresVerification && body.promoVerified !== true) return NextResponse.json({ success: false, error: 'This promotion requires cashier verification.' }, { status: 400 });
    ...
    const transaction = await Transaction.create({ transactionNo, customerName: body.customerName || '', plate: String(body.plate).trim().toUpperCase(), vehicleType: body.vehicleType, vehicleSize: body.vehicleSize || undefined, services: body.services, subtotal, discount, promoId, promoName, promoType, total, paymentMethod, amountPaid, change, createdBy: user.id, status: 'completed' });
    for (const entry of deductions) { const before = Number(entry.item.quantity), after = before - entry.quantity; await InventoryItem.updateOne({ _id: entry.item._id }, { $set: { quantity: after } }); await InventoryMovement.create({ itemId: entry.item._id, itemName: entry.item.name, type: 'sale', quantity: -entry.quantity, beforeQuantity: before, afterQuantity: after, transactionId: transaction._id, transactionNo, userId: user.id, reason: 'POS sale' }); }
    ...
    if (customer) { const vehicle = customer.vehicles.find((v: any) => v.plate === plate); if (vehicle) { vehicle.vehicleType = body.vehicleType; vehicle.vehicleSize = body.vehicleSize || vehicle.vehicleSize; vehicle.visitCount = Number(vehicle.visitCount || 0) + 1; vehicle.lastVisitAt = now; } else customer.vehicles.push({ plate, vehicleType: body.vehicleType, vehicleSize: body.vehicleSize, visitCount: 1, lastVisitAt: now }); customer.totalVisits = Number(customer.totalVisits || 0) + 1; customer.lastVisitAt = now; if (customer.name === 'Walk-in Customer' && customerName !== 'Walk-in Customer') { customer.name = customerName; customer.normalizedName = normalizedName; } await customer.save(); }
