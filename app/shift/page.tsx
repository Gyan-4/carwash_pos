...
type Shift = {
  ...
  expectedCash?: number; summary?: { cashSales: number; gcashSales: number; cardSales: number; sales: number; transactions: number };
};
...
export default function ShiftPage() {
  ...
  const openModal = (next: Modal) => { setAmount(''); setReason(''); setMessage(''); setError(''); setModal(next); };
  ...
  async function submit() {
    ...
    try {
      ...
      if (action === 'open') payload.openingCash = Number(amount);
      if (action === 'cash-in' || action === 'cash-out') { payload.shiftId = active?._id; payload.amount = Number(amount); payload.reason = reason.trim(); }
      if (action === 'close') { payload.shiftId = active?._id; payload.actualCash = Number(amount); payload.closingNote = reason.trim(); }
      ...
      setModal(null); setMessage(action === 'open' ? 'Shift opened successfully.' : action === 'close' ? 'Shift closed successfully.' : 'Cash movement recorded.'); await load();
  ...
  return (
    <div className="h-full overflow-y-auto bg-slate-50/60 p-6 text-slate-900 font-sans">
      <div className="mx-auto max-w-6xl space-y-5">
        <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-blue-600">Cashier Operations</p><h1 className="mt-1 text-xl font-black text-slate-950">Cashier Shift</h1><p className="mt-1 text-xs text-slate-500">Track opening cash, cash movements, sales, and end-of-shift variance.</p></div>
          <button onClick={load} className="flex items-center justify-center gap-2 rounded-xl border border-slate-300 px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50"><RefreshCw className="h-4 w-4" /> Refresh</button>
        </div>
        ...
        {!active ? (
          ...
          <>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <Stat icon={Clock3} label="Expected Drawer" value={peso(expected)} />
            </div>

            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-emerald-500" /><h2 className="text-sm font-black">Shift Open</h2></div><p className="mt-1 text-xs text-slate-500">{active.cashierName} · opened {dateTime(active.openedAt)}</p></div><div className="flex flex-wrap gap-2"><button onClick={() => openModal('cash-in')} className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700 hover:bg-emerald-100">Cash In</button><button onClick={() => openModal('cash-out')} className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-black text-amber-700 hover:bg-amber-100">Cash Out</button><button onClick={() => openModal('close')} className="rounded-xl bg-slate-950 px-3 py-2 text-xs font-black text-white hover:bg-slate-800"><LockKeyhole className="mr-1 inline h-3.5 w-3.5" />Close Shift</button></div></div>
              <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-5"><Metric label="Transactions" value={String(active.summary?.transactions || 0)} /><Metric label="Total Sales" value={peso(active.summary?.sales || 0)} /><Metric label="Cash Sales" value={peso(active.summary?.cashSales || 0)} /><Metric label="GCash" value={peso(active.summary?.gcashSales || 0)} /><Metric label="Card" value={peso(active.summary?.cardSales || 0)} /></div>
            </div>
          </>
        )}
