...
import { KeyRound, Pencil, Plus, ShieldCheck, UserCheck, UserX, Users, X } from 'lucide-react';
...
export default function UsersPage() {
  ...
  function openCreate() {
    setSelected(null); setName(''); setRole('cashier'); setPin(''); setActive(true); setMessage(''); setModal('create');
  }
  function openEdit(user: User) {
    setSelected(user); setName(user.name); setRole(user.role); setActive(user.active); setPin(''); setMessage(''); setModal('edit');
  }
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      const body = modal === 'create' ? { name, role, pin } : modal === 'pin' ? { id: selected?.id, pin } : { id: selected?.id, name, role, active };
      const response = await fetch('/api/users', { method: modal === 'create' ? 'POST' : 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Unable to save user.');
      setModal(null); setMessage(modal === 'create' ? 'User created successfully.' : modal === 'pin' ? 'PIN updated successfully.' : 'User updated successfully.');
      await load();
    } catch (e) { setMessage(e instanceof Error ? e.message : 'Unable to save user.'); }
    ...
    <div className="h-full w-full overflow-y-auto bg-slate-50/60 p-6 font-sans text-slate-900">
      <div className="mx-auto max-w-6xl space-y-5">
        <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="flex items-center gap-2"><Users className="h-5 w-5 text-blue-600" /><h1 className="text-lg font-black">User Management</h1></div>
            <p className="mt-1 text-xs text-slate-600">Manage cashier and manager accounts, access status, and 4-digit terminal PINs.</p>
          </div>
            ...
            <div key={user.id} className={`rounded-2xl border bg-white p-5 shadow-sm ${user.active ? 'border-slate-200' : 'border-slate-300 opacity-70'}`}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-center gap-3">
                  <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${user.role === 'manager' ? 'bg-purple-50 text-purple-600' : 'bg-blue-50 text-blue-600'}`}>{user.role === 'manager' ? <ShieldCheck className="h-5 w-5" /> : <UserCheck className="h-5 w-5" />}</div>
                  <div className="min-w-0"><h2 className="truncate text-sm font-black">{user.name}</h2><p className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500">{user.role}</p></div>
                </div>
              ...
              <button onClick={() => { setSelected(user); setName(user.name); setRole(user.role); setActive(!user.active); setPin(''); setModal('edit'); }} className={`mt-2 flex w-full items-center justify-center gap-1.5 rounded-lg border px-3 py-2 text-[11px] font-extrabold ${user.active ? 'border-red-200 text-red-700 hover:bg-red-50' : 'border-emerald-200 text-emerald-700 hover:bg-emerald-50'}`}>
                {user.active ? <UserX className="h-3.5 w-3.5" /> : <UserCheck className="h-3.5 w-3.5" />} {user.active ? 'Deactivate' : 'Activate'}
              </button>
            </div>
          ))}
        </div>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" onMouseDown={(e) => e.target === e.currentTarget && setModal(null)}>
          <form onSubmit={submit} className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            ...
            {modal !== 'pin' && <div className="space-y-4">
              <div><label className="mb-1 block text-[10px] font-extrabold uppercase text-slate-500">Name</label><input required maxLength={100} value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2.5 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
              <div><label className="mb-1 block text-[10px] font-extrabold uppercase text-slate-500">Role</label><select value={role} onChange={(e) => setRole(e.target.value as 'cashier' | 'manager')} className="w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2.5 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500"><option value="cashier">Cashier</option><option value="manager">Manager</option></select></div>
              <label className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 p-3"><span><span className="block text-xs font-extrabold">Account status</span><span className="text-[10px] text-slate-500">Inactive users cannot sign in.</span></span><input type="checkbox" checked={active} onChange={(e) => setActive(e.target.checked)} className="h-4 w-4 accent-blue-600" /></label>
            </div>}
